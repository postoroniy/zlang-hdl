...
def test_advertised_capability_witness_is_semantically_executable() -> None:
    def check(capability) -> None:
        witness = capability.witness
        path = ROOT / witness.source_path
        assert path.is_file(), (
            f"missing witness for {capability.name}: {witness.source_path}"
        )
        result = _compile_witness(witness.source_path, witness.top)
        assert result.ir.name == witness.top

    check_cases(
        ((item.name, item) for item in CAPABILITY_REGISTRY.capabilities),
        check,
        matrix="capabilities",
    )
...
INTRINSIC_FAMILY_WITNESSES = (
    ...
    (
        "fixed-conversion",
        "module M { in x:fixed<8,4> out y:fixed<8,2> y=fixed_truncate_wrap(x) }",
    ),
...
def test_representative_registered_intrinsic_families_compile() -> None:
    ...
    check_cases(
        ((family, (family, source)) for family, source in INTRINSIC_FAMILY_WITNESSES),
        check,
        matrix="intrinsic_families",
    )
...
def test_exploration_capability_distinguishes_advisory_and_required_formal() -> None:
    capability = next(
        item for item in CAPABILITY_REGISTRY.capabilities if item.name == "exploration"
    )
    assert "`available` is advisory" in capability.formal
    assert "required policies require connected semantic-reference equivalence" in capability.formal
    assert any("formal-aware selection" in item for item in capability.limitations)

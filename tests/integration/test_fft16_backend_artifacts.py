...
def test_fft16_artifacts_are_deterministic_and_preserve_four_roms() -> None:
    module = _compile().ir
    design = build_recursive_formal_design(module)
    expected_paths = {
        (TOP,),
    ...
    for artifact, emitter in zip(
        ...
        assert sorted(item.depth for item in artifact.companions) == [1, 2, 4, 8]
        assert {item.word_width for item in artifact.companions} == {32}
        assert len({item.semantic_id for item in artifact.companions}) == 4
        assert len({item.content_hash for item in artifact.companions}) == 4
...
def test_fft16_clash_dispatches_exact_helpers_and_rom_shapes() -> None:
    module = _compile().ir
    helpers = {
        item.instance.name: (
    ...
    assert (
        ...
        "parent_input stage_d4_input_ready"
    ) in clash
    assert (
...
def test_fft16_real_clash_generation_and_established_rom_lint_waiver(
    tmp_path: Path,
) -> None:
    module = _compile().ir
    ...
    lint = subprocess.run(
        (
            ...
            "-Wno-UNUSEDPARAM",
            "-Wno-UNDRIVEN",
            # Clash 1.11 romFile indexes its Verilog ROM with a host-width
            # Int.  This established waiver leaves arithmetic widths fatal.
            "-Wno-WIDTHTRUNC",
            str(rtl),
        ),
        cwd=rtl.parent,

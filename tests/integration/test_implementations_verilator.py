...
class ImplementationChoiceVerilatorTests(unittest.TestCase):
    @unittest.skipUnless(
    ...
    def test_selected_dsp_mac_runs_with_declared_latency_in_verilator(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            harness = root / "mac_choice_test.cpp"
            harness.write_text(
                "#include \"VMacChoice.h\"\n"
                "static void tick(VMacChoice& d) {\n"
                "  d.clk = 0; d.eval(); d.clk = 1; d.eval(); d.clk = 0; d.eval();\n"
                "}\n"
                "int main() {\n"
                "  VMacChoice d; d.clk = 0; d.rst = 1;\n"
            ...
            subprocess.run([str(object_directory / "mac_choice_sim")], check=True)

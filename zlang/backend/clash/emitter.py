...
from zlang.ir.storage import MemoryCollision, RomSignal
...
def _clash_instance_name(name: str) -> str:
    ...
    return f"zlang_instance_{match.group(1)}_{match.group(2)}_{digest}"
...
def _clash_recursive_token_overrides(module: Module) -> dict[str, str]:
    ...
    for connection in module.request_response_connections:
        ...
        base = (
            f"rr_{_clash_instance_name(source.owner)}_"
            f"{_clash_name(source.name)}_"
            f"{_clash_instance_name(connection.response.destination.owner)}_"
            "outstanding"
        )
...
def _emit_elastic_pipeline_component(module: Module, function_name: str) -> str:
    ...
    delay_names = {
        _leaf_key(node): f"{_stage_prefix(node)}_{node.instance}_s{node.stages}"
    ...
    for node in staged.values():
        ...
        for stage in range(1, node.stages + 1):
            stage_name = f"{_stage_prefix(node)}_{node.instance}_s{stage}"
            bindings.append(
...
def _emit_csr_module(module: Module) -> str:
    ...
    if has_user_transition or has_user_assignments:
        ...
        for output in module.outputs:
            ...
            writers: list[tuple[object, object]] = []
            for group in groups:
                rule = rules_by_name[group.rule_name]
                action = next(
                    (
                        item for item in rule.actions
            ...
            for group, action in reversed(writers):
                scheduled = expr.Mux(
                    expr.InputRef(
                        f"rule_{group.rule_name}_fire", BitType()
                    ),
                    action.expression,
                    scheduled,
                    output.type,
...
def _emit_storage_circuit(
    module: Module, function_name: str = "circuit"
    ...
    extensions, imports, declarations = _emit_prelude(module)
    if module.children:
        declarations += _emit_scalar_child_declarations(module.children)
    ...
    for instance, child in zip(
        ...
        function = (
            _protocol_child_name(child)
            if child.csr_blocks
            else _child_function_name(child)
        )
        ...
        child_outputs = [
            f"{_clash_instance_name(instance.name)}_"
...
def _emit_unified_schedule_bindings(module: Module, render=None) -> tuple[str, ...]:
    ...
    lines = [
        f"rule_{group.rule_name}_guard = {render(group.guard)}"
        for group in groups
    ]
    count_names = [fifo.name for fifo in module.fifos]
    guard_names = [group.rule_name for group in groups]
    parameters = [
        *(f"count_{name}" for name in count_names),
        *(f"guard_{name}" for name in guard_names),
        "resetActive",
    ]
    applications = [
        *(f"{name}_count" for name in count_names),
        *(f"rule_{name}_guard" for name in guard_names),
        "reset_active",
    ]
    for selected_group in groups:
        ...
        for region in selection_regions(transition, selected_group.rule_name):
            ...
            guard_values = region[len(count_names):]
            terms: list[str] = []
            for name, fifo, value in zip(
                count_names, module.fifos, count_values, strict=True
            ):
            ...
            terms.extend(
                f"guard_{name} == {'high' if value else 'low'}"
        ...
        lines.append(
            f"rule_{selected_group.rule_name}_fire = ({lambda_}if not resetActive && ({condition}) then high else low){application}"
        )
    return tuple(lines)


def _emit_unified_register_bindings(module: Module, render=None) -> tuple[str, ...]:
    ...
    for register in module.registers:
        ...
        for group in reversed(groups):
            action = next(
                (item for item in group.actions if item.resource_id == resource_id),
                None,
            )
            if action is not None:
                scheduled = expr.Mux(
                    expr.InputRef(f"rule_{group.rule_name}_fire", BitType()),
...
def _emit_scheduled_fifo_bindings(module: Module, fifo: object, render=None) -> tuple[str, ...]:
    ...
    push_groups = [
        ...
        if any(item.resource_id == resource_id and item.kind is StateActionKind.FIFO_PUSH for item in group.actions)
    ]
    pop_groups = [
        group for group in groups
        if any(item.resource_id == resource_id and item.kind is StateActionKind.FIFO_POP for item in group.actions)
    ]

    def or_fires(items: list[object]) -> str:
        if not items:
            return "pure low"
        value = f"rule_{items[0].rule_name}_fire"
        for item in items[1:]:
            ...
            value = f"((.|.) <$> ({value}) <*> rule_{item.rule_name}_fire)"
        return value
    ...
    for group in reversed(push_groups):
        action = next(
            ...
            if item.resource_id == resource_id and item.kind is StateActionKind.FIFO_PUSH
        )
        payload = (
            f"(\\fire value fallback -> if fire == high then value else fallback) "
            f"<$> rule_{group.rule_name}_fire "
    ...
    return (
        ...
        f"{name}_enqueue = {or_fires(push_groups)}",
        f"{name}_dequeue = {or_fires(pop_groups)}",
...
def _emit_memory_bindings(memory: object, render=None) -> tuple[str, ...]:
    ...
    if memory.write_mask_width is not None:
        ...
        effective_mask = (
            ...
            f"(unpack mask :: Vec {memory.write_mask_width} Bit)) :: BitVector {memory.element_type.width})"
        )
        merged = (
...
def _emit_scheduled_memory_bindings(module: Module, memory: object, render=None) -> tuple[str, ...]:
    ...
    for group in reversed(write_groups):
        ...
        fire = f"rule_{group.rule_name}_fire"
        write_address = (
    ...
    read_value = (
        ...
        + f") <$> {name}_cells <*> {name}_read_address <*> {name}_write_fire "
        f"<*> {name}_write_address <*> {effective_write_data}"
    )
    bindings = [
        f"{name}_read_fire = {or_fires(read_groups)}",
        f"{name}_write_fire = {or_fires(write_groups)}",
        f"{name}_read_address = {read_address}",
        f"{name}_write_address = {write_address}",
        f"{name}_write_data = {write_data}",
    ]
    ...
    bindings.extend((
        read_value,
        f"{name}_read_data = register {zero} {name}_read_data_next",
        f"{name}_read_data_next = (\\fire value old -> if fire == high then value else old) "
        f"<$> {name}_read_fire <*> {name}_read_value <*> {name}_read_data",
        ...
        f"writeData cells else cells) <$> {name}_cells <*> {name}_write_fire "
        f"<*> {name}_write_address <*> {effective_write_data}",
    ))
...
def _emit_connection_module(
    ...
    *,
    ...
    if module.registers or module.next_assignments:
        raise ClashEmissionError(
            "connection modules with user registers are not implemented"
        )
...
def _emit_protocol_child_function(
    ...
    nested_csr = "\n".join(
        _emit_csr_child_function(item)
        for item in child.children if item.csr_blocks
    )
    prefix = nested_csr + ("\n" if nested_csr else "")
    if child.csr_blocks:
    ...
    if child.fifos and child.resolved_transition is not None:
        ...
        nested_scalar_declarations = _emit_scalar_child_declarations(
            tuple(item for item in child.children if not item.csr_blocks)
        )
        wire_inputs = [
...
def _emit_recursive_protocol_child_function(
    ...
    def owned_signal(owner: str, port: str) -> str:
        return f"{_clash_instance_name(owner)}_{_clash_name(port)}"
    ...
    def backward(endpoint: ProtocolEndpoint) -> str:
        ...
        return f"{owned_signal(endpoint.owner, endpoint.name)}_ready"
    ...
    for instance in child.instances:
        ...
        result_name = f"{instance_name}_result"
    ...
    if has_local_state:
        ...
        groups = ordered_state_groups(transition)
        next_by_register = {
            item.target.name: item.expression
            for item in child.next_assignments
        }
        for register in child.registers:
            register_name = _clash_name(register.name)
            bindings.append(
                f"{register_name} = register "
                f"{_emit_expression(register.initial)} "
                f"{register_name}_next"
            )
            resource_id = next(
                (
                    item.semantic_id for item in transition.resources
                    if item.kind.value == "register"
                    and item.name == register.name
                ),
                None,
            )
            if resource_id is None:
                raise ClashEmissionError(
                    f"recursive ready/valid child '{child.name}' register "
                    f"'{register.name}' is absent from its resolved transition"
                )
            scheduled_expression: expr.Expression = next_by_register.get(
                register.name,
            ...
            for group in reversed(groups):
                action = next(
                    (
                        item for item in group.actions
                        if item.resource_id == resource_id
                        and item.kind is StateActionKind.REGISTER_WRITE
                    ),
                    None,
                )
                if action is not None:
            ...
            bindings.append(
                f"{register_name}_next = "
                f"{render_expression(scheduled_expression)}"
            )
...
def _emit_mixed_protocol_child_function(
    ...
    bindings: list[str] = []
    bindings.extend(f"{p.name} = mixed_{p.name}" for p in (*wire_inputs, *rv_inputs))
    ...
    for instance in child.instances:
        ...
        result = f"{instance.name}_csr_result"
        bindings.append(
            f"{result} = {_protocol_child_name(nested)} "
    ...
    delay_names = {_leaf_key(d): f"{_stage_prefix(d)}_{d.instance}_s{_stage_count(d)}" for d in delay_nodes.values()}
    ordered_rules = _rule_schedule(child)
    for rule in ordered_rules:
        fire = f"rule_{rule.name}_fire"
        raw = _emit_signal_expression(rule.guard, {**signal_names, **delay_names})
        bindings.append(
        ...
        for rule in reversed(ordered_rules):
            for action in rule.actions:
                if action.target.name == register.name:
                    scheduled = expr.Mux(expr.InputRef(f"rule_{rule.name}_fire", BitType()), action.expression, scheduled, register.type)
        bindings.append(f"{register.name}_next = {_emit_signal_expression(scheduled, {**signal_names, **delay_names})}")
    for delay in delay_nodes.values():
        ...
        for stage in range(1, _stage_count(delay) + 1):
            name = f"{_stage_prefix(delay)}_{delay.instance}_s{stage}"
    ...
    for port in wire_outputs:
        ...
        values.append(_emit_signal_expression(scheduled, {**signal_names, **delay_names}))
...
def _request_response_component_output_name(instance_name: str) -> str:
...
def _emit_request_response_child_function(
    ...
    *,
    ...
    wire_inputs = [p for p in child.inputs if p.protocol is InterfaceProtocol.WIRE]
    wire_outputs = [p for p in child.outputs if p.protocol is InterfaceProtocol.WIRE]
    input_type = _component_type_name(child, "Input")
    output_type = _component_type_name(child, "Output")
    name = interface.name
    def field(channel: str, suffix: str) -> str:
        return f"{_component_accessor(child, name, channel.capitalize() + suffix)}"
    ...
    input_fields = [(_component_accessor(child, p.name), _emit_type(p.type)) for p in wire_inputs]
    output_fields: list[tuple[str, str]] = []
    ...
    output_fields += [(_component_accessor(child, p.name), _emit_type(p.type)) for p in wire_outputs]
    ...
    component_input = "componentInput"
    names = {
        _leaf_key(expr.InputRef(p.name, p.type)): f"({_component_accessor(child, p.name)} {component_input})"
        for p in wire_inputs
    }
    if len(child.registers) > 1:
        raise ClashEmissionError(
        ...
        for selected in groups:
            ...
            for region in selection_regions(
                ...
                terms = [
                    ...
                    for group, value in zip(groups, region, strict=True)
                    if value is not None
                ]
                clauses.append("(" + " && ".join(terms) + ")")
            condition = " || ".join(clauses) if clauses else "False"
...
def _emit_hierarchical_ordinary_protocol_module(module: Module) -> str:
    ...
    def forward(endpoint: ProtocolEndpoint) -> str:
        ...
        return (
            f"{_clash_instance_name(endpoint.owner)}_"
            f"{_clash_name(endpoint.name)}"
        )
    ...
    def backward(endpoint: ProtocolEndpoint) -> str:
        ...
        return (
            f"{_clash_instance_name(endpoint.owner)}_"
            f"{_clash_name(endpoint.name)}_ready"
        )
    ...
    for index, instance in enumerate(module.instances):
        ...
        result = f"{_clash_instance_name(instance.name)}_result"
        ...
        for position, port in enumerate(result_ports):
            if port.protocol is InterfaceProtocol.READY_VALID and port.direction is PortDirection.INPUT:
                ...
                bindings.append(
                    f"{_clash_instance_name(instance.name)}_"
                    f"{_clash_name(port.name)} = {projection(position)}"
                )
    ...
    for port in wire_outputs:
        ...
        bindings.append(
            f"{_clash_name(port.name)} = "
            f"{_emit_signal_expression(assignment.expression, parent_signal_names)}"
        )
...
def _emit_hierarchical_protocol_module(module: Module) -> str:
    ...
    delegated = {
        ...
        if connection.delegation and "." in connection.destination
    }
    ...
    for local in module.locals:
        bindings.append(
            f"{local.name} = {_emit_signal_expression(local.expression, {})}"
        )
    if module.registers or module.rules:
        bindings.append("reset_active = unsafeToActiveHigh hasReset")
    ordered_rules = _rule_schedule(module)
    for rule in ordered_rules:
        raw = _emit_signal_expression(rule.guard, {})
        bindings.append(
            f"rule_{rule.name}_fire = (\\guard resetActive -> if resetActive then low else guard) "
            f"<$> {_clash_apply_arg(raw)} <*> reset_active"
        )
    ...
    def emit_delegated_outputs(instance_name: str, child: Module) -> None:
        for port in child.ports:
            ...
            if member.protocol is InterfaceProtocol.READY_VALID:
                if port.direction is PortDirection.INPUT:
                    ready_signal = f"{instance_name}_{port.name}_ready"
                    bindings.append(
                        f"{_top_leaf_signal(top_leaves[top_path + ('ready',)])} = "
                        f"zlangRvReady <$> {ready_signal}"
                    )
                else:
                    forward_signal = f"{instance_name}_{port.name}"
                ...
                bindings.append(f"{_top_leaf_signal(leaf)} = {instance_name}_{port.name}")
    extensions, imports, declarations = _emit_prelude(module)
    declarations = _ready_valid_declarations() + declarations
    declarations += _domain_declaration(module, "ZLangSystem") + "\n\n"
    child_text = "\n".join(
    ...
    def rr_endpoint_name(endpoint: object, channel: str) -> str:
        return (
            f"{_clash_instance_name(endpoint.owner)}_"
            f"{_clash_name(endpoint.name)}_{channel}"
        )
    ...
    for descriptor in module.request_response_connections:
        ...
        tracker = (
            f"rr_{_clash_instance_name(request.owner)}_"
            f"{_clash_name(request.name)}_"
            f"{_clash_instance_name(descriptor.response.destination.owner)}_"
            "outstanding"
        )
    ...
    for instance in module.instances:
        ...
        if child.request_responses:
            ...
            interface = child.request_responses[0]
            name = interface.name
            instance_token = _clash_instance_name(instance.name)
            wire_inputs = [p for p in child.inputs if p.protocol is InterfaceProtocol.WIRE]
            wire_outputs = [p for p in child.outputs if p.protocol is InterfaceProtocol.WIRE]
            ...
            if interface.role is RequestResponseRole.REQUESTER:
                rr_args = [
                    f"{instance_token}_{_clash_name(name)}_request_ready",
                    f"{instance_token}_{_clash_name(name)}_response",
                ]
            else:
                rr_args = [
                    f"{instance_token}_{_clash_name(name)}_request",
                    f"{instance_token}_{_clash_name(name)}_response_ready",
                ]
            args = scalar_args + rr_args
            input_type = _component_type_name(child, "Input")
            output_type = _component_type_name(child, "Output")
            component_input = f"{instance_token}_component_input"
            component_output = _request_response_component_output_name(instance.name)
            bindings.append(
            ...
            bindings.append(
                f"{component_output} = "
                f"{_protocol_instance_child_name(module, instance.name, child)} "
                f"{component_input}"
            )
            output_fields: list[tuple[str, str]] = [
                (
                    f"{instance_token}_{_clash_name(p.name)}",
                    _component_accessor(child, p.name),
                )
                for p in wire_outputs
            ]
            if interface.role is RequestResponseRole.REQUESTER:
                output_fields += [
                    (f"{instance_token}_{_clash_name(name)}_request", _component_accessor(child, name, "RequestForward")),
                    (f"{instance_token}_{_clash_name(name)}_response_ready", _component_accessor(child, name, "ResponseBackward")),
                ]
            else:
                output_fields += [
                    (f"{instance_token}_{_clash_name(name)}_request_ready", _component_accessor(child, name, "RequestBackward")),
                    (f"{instance_token}_{_clash_name(name)}_response", _component_accessor(child, name, "ResponseForward")),
                ]
        ...
        for port in ordered_inputs:
            if port.protocol is InterfaceProtocol.READY_VALID:
                ...
                if delegated_input is not None and (instance.name, port.name) in delegated_children:
                    ...
                    args.append(f"{edge.source.owner}_{edge.source.name}")
            elif port.protocol is InterfaceProtocol.WIRE:
                edge = next((c for c in incoming if c.destination.name == port.name), None)
                if edge is not None:
                    args.append(f"{edge.source.owner}_{edge.source.name}")
        ...
        for port in child.outputs:
            if port.protocol is InterfaceProtocol.READY_VALID:
                ...
                if delegated_backward is not None and (instance.name, port.name) in delegated_children:
                    ...
                    args.append(f"{instance.name}_{port.name}_ready")
        if _uses_bundled_component_abi(child):
            input_type = _component_type_name(child, "Input")
            component_input = f"{instance.name}_component_input"
            component_output = f"{instance.name}_component_output"
            ...
            for port in child.inputs:
                if port.protocol is InterfaceProtocol.READY_VALID:
                    accessor = _component_accessor(child, port.name, "Backward")
                    bindings.append(
                        f"{instance.name}_{port.name}_ready = {accessor} <$> {component_output}"
                    )
            for port in child.outputs:
                suffix = "Forward" if port.protocol is InterfaceProtocol.READY_VALID else ""
                accessor = _component_accessor(child, port.name, suffix)
                bindings.append(
                    f"{instance.name}_{port.name} = {accessor} <$> {component_output}"
                )
            emit_delegated_outputs(instance.name, child)
            continue
        call = f"{_protocol_instance_child_name(module, instance.name, child)}" + (
        ...
        if result_count == 1:
            result = f"{instance.name}_{child.outputs[0].name}"
            bindings.append(f"{result} = {call}")
        else:
            result = f"{instance.name}_result"
            bindings.append(f"{result} = {call}")
        ...
        for index, port in enumerate(result_ports):
            if port.protocol is InterfaceProtocol.READY_VALID and port.direction is PortDirection.INPUT:
                bindings.append(f"{instance.name}_{port.name}_ready = {projection(index)}")
        for index, port in enumerate(result_ports):
            if port.protocol is InterfaceProtocol.READY_VALID and port.direction is PortDirection.OUTPUT:
                lhs = f"{instance.name}_{port.name}"
                rhs = projection(index)
                if lhs != rhs:
                    bindings.append(f"{lhs} = {rhs}")
        for index, port in enumerate(result_ports):
            if port.protocol is InterfaceProtocol.WIRE:
                bindings.append(f"{instance.name}_{port.name} = {projection(index)}")
        emit_delegated_outputs(instance.name, child)
    ...
    for connection in module.hierarchical_connections:
        if connection.source.channel is not None:
            ...
            continue
        source_name = f"{connection.source.owner}_{connection.source.name}"
        destination_name = (
            connection.destination.name
            if connection.destination.owner == module.name
            else f"{connection.destination.owner}_{connection.destination.name}"
        )
        source_ready = f"{source_name}_ready"
        destination_backward = (
            f"{connection.destination.name}_backward"
            if connection.destination.owner == module.name
            else f"{destination_name}_ready"
        )
        if connection.source.protocol is InterfaceProtocol.WIRE:
    ...
    for leaf in top_abi.outputs:
        ...
        rendered = _emit_signal_expression(
            assignment.expression, parent_signal_names
        )
    ...
    for output in ordinary_outputs:
        if output.protocol is InterfaceProtocol.READY_VALID:
            bindings.append(f"{output.name}_ready = zlangRvReady <$> {output.name}_backward")
            protocol_output_arguments.append(f"{output.name}_backward")
            protocol_output_signatures.append("Signal ZLangSystem ZLangReadyValidBackward")
    for output in ordinary_outputs:
        if output.protocol is InterfaceProtocol.READY_VALID:
            output_values.append(output.name)
        ...
        bindings.append(f"{output.name} = {_emit_signal_expression(assignment.expression, {})}")
        output_values.append(output.name)
    ...
    parent_inputs = ordinary_inputs
    aggregate_input_leaves = top_abi.inputs
    input_signature = [f"Signal ZLangSystem ({_emit_type(p.type)})" for p in parent_inputs]
    ...
    circuit_arguments = " ".join((
        *(f"parent_{_clash_name(p.name)}" for p in parent_inputs),
        *aggregate_arguments,
...
def _emit_request_response_module(module: Module) -> str:
    ...
    if module.registers or module.next_assignments:
        raise ClashEmissionError(
            "request/response modules with user registers are not implemented"
        )
    staged: dict[int, expr.Delay | expr.Pipeline] = {}
    for assignment in module.assignments:
        _collect_delays(assignment.expression, staged)
    if staged:
        raise ClashEmissionError(
            "staged expressions in request/response modules are not implemented"
        )
    ...
    for assignment in module.assignments:
        bindings.append(
            ...
            f"{_emit_signal_expression(assignment.expression, value_names)}"
        )
    for interface in module.request_responses:
...
def _child_function_name(module: Module) -> str:
    return _clash_name(module.name[:1].lower() + module.name[1:])
...
def _emit_combinational_hierarchy(module: Module) -> str:
    ...
    for instance, child in zip(module.instances, module.children, strict=True):
        ...
        arguments = " ".join(
            f"({_emit_expression(child_bindings[port.name])})"
            for port in child.inputs
        )
        application = _child_function_name(child)
        if arguments:
            application += " " + arguments
        child_outputs = [
            f"{_clash_instance_name(instance.name)}_{_clash_name(port.name)}"
            for port in child.outputs
        ]
    ...
    body = _emit_scalar_product([
        _emit_expression(output_drivers[output.name]) for output in outputs
    ])
    where = "\n".join(f"  {item}" for item in bindings)
    input_ports = ", ".join(
        _top_value_port_annotation(port.name, port.type) for port in inputs
...
def _emit_child_function(module: Module) -> str:
    ...
    function_name = _child_function_name(module)
    materialized = plan_materialization(
        (output_expression,),
        reserved_names=(
            function_name,
    ...
    aliases = {item.expression: item.name for item in materialized}
    body = _emit_expression(replace_materialized(output_expression, aliases))
    ...
    nested_declarations = (
        _emit_scalar_child_declarations(module.children) + "\n"
...
def _emit_child_sequential(module: Module) -> str:
    """Emit a stateful child as a hidden-clock/reset signal function."""
    if len(module.outputs) != 1:
        raise ClashEmissionError(
            f"sequential child '{module.name}' requires one wire output"
        )
    domain = "ZLangSystem"
    inputs = module.inputs
    signature = " -> ".join(
        [*(f"Signal {domain} ({_emit_type(port.type)})" for port in inputs),
         f"Signal {domain} ({_emit_type(module.outputs[0].type)})"]
    )
    args = " ".join(_clash_name(port.name) for port in inputs)
    child_declarations = ""
    if module.children:
        child_declarations = _emit_scalar_child_declarations(module.children) + "\n"
    bindings: list[str] = ["reset_active = unsafeToActiveHigh hasReset"]
    delay_nodes: dict[int, expr.Delay | expr.Pipeline] = {}
    ...
    delay_names = {
        _leaf_key(value): (
            f"{_stage_prefix(value)}_{value.instance}_s{_stage_count(value)}"
        )
    ...
    for instance in module.instances:
        child = next((item for item in module.children if item.name == instance.module), None)
        if child is None:
            raise ClashEmissionError(f"missing elaborated child module '{instance.module}'")
        child_bindings = {
            item.port: item.expression
        ...
        function = (
            _protocol_child_name(child) if child.csr_blocks
            else _child_function_name(child)
        )
        application = _emit_child_application(
    ...
    ordered_rules = _rule_schedule(module)
    for rule in ordered_rules:
        raw = _emit_signal_expression(rule.guard, delay_names)
        bindings.append(
            f"rule_{rule.name}_fire = (\\guard resetActive -> if resetActive then low else guard) "
            f"<$> {_clash_apply_arg(raw)} <*> reset_active"
        )
    ...
    for value in delay_nodes.values():
        ...
        for stage in range(1, _stage_count(value) + 1):
            stage_name = f"{_stage_prefix(value)}_{value.instance}_s{stage}"
            bindings.append(
                f"{stage_name} = register {_zero_value(value.type)} ({previous})"
            )
            previous = stage_name
    output = module.assignments[0].expression if module.assignments else expr.Constant(0, module.outputs[0].type)
    bindings.append(
        f"{_clash_name(module.outputs[0].name)} = "
        f"{_emit_signal_expression(output, delay_names)}"
    )
    body = "\n".join(f"  {item}" for item in bindings)
    return (
        f"{_child_function_name(module)} :: HiddenClockResetEnable {domain} => {signature}\n"
        f"{_child_function_name(module)} {args} = {_clash_name(module.outputs[0].name)}\n"
        f" where\n{body}\n{child_declarations}"
    )


def _emit_scalar_child_declarations(children: tuple[Module, ...]) -> str:
    declarations: list[str] = []
    seen: set[str] = set()
    for child in children:
        function = (_protocol_child_name(child) if child.csr_blocks
                    else _child_function_name(child))
        if function in seen:
            continue
        seen.add(function)
        if child.csr_blocks:
            declarations.append(_emit_csr_child_function(child))
        elif child.fifos or child.memories or child.roms:
            declarations.append(
                _emit_storage_circuit(child, function).circuit
            )
        elif child.is_sequential:
            declarations.append(_emit_child_sequential(child))
        else:
            declarations.append(_emit_child_function(child))
    return "\n".join(declarations)
...
def _emit_sequential_module(module: Module) -> str:
    ...
    delay_names = {
        _leaf_key(delay): (
            f"{_stage_prefix(delay)}_{delay.instance}_s{_stage_count(delay)}"
        )
        for delay in delay_nodes.values()
    }
    ...
    for delay in delay_nodes.values():
        ...
        previous = source
        for stage in range(1, _stage_count(delay) + 1):
            stage_name = f"{_stage_prefix(delay)}_{delay.instance}_s{stage}"
...
def _signal_leaves(
    ...
    def visit(node: expr.Expression) -> None:
        if isinstance(node, (expr.InputRef, expr.RegisterRef)):
            ...
            found.append(
                (_leaf_key(node), f"{_clash_instance_name(node.instance)}_{_clash_name(node.port)}")
            )

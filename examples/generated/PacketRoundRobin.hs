...
circuit source_a source_b tx_backward = (ZLangPacketBackward <$> source_a_ready, ZLangPacketBackward <$> source_b_ready, ZLangPacketForward <$> tx_payload <*> tx_valid <*> tx_last)
 where
  ...
  candidate = (\priority valid_0 valid_1 -> case priority of { 0 -> if valid_0 == high then 0 else if valid_1 == high then 1 else 0; 1 -> if valid_1 == high then 1 else if valid_0 == high then 0 else 0; _ -> 0 }) <$> next_priority <*> source_a_valid <*> source_b_valid
  ...
  selected_valid_raw = (\selected valid_0 valid_1 -> case selected of { 0 -> valid_0; 1 -> valid_1; _ -> valid_0 }) <$> selected <*> source_a_valid <*> source_b_valid
  selected_last = (\selected last_0 last_1 -> case selected of { 0 -> last_0; 1 -> last_1; _ -> last_0 }) <$> selected <*> source_a_last <*> source_b_last
  selected_payload = (\selected payload_0 payload_1 -> case selected of { 0 -> payload_0; 1 -> payload_1; _ -> payload_0 }) <$> selected <*> source_a_payload <*> source_b_payload
  ...
  next_priority = register (0 :: Unsigned 1) next_priority_next
  next_priority_next = (\priority selected complete -> if complete == high then if selected == 1 then 0 else selected + 1 else priority) <$> next_priority <*> selected <*> grant_complete
...
topEntity clk rst source_a source_b tx_backward = exposeClockResetEnable circuit clk rst enableGen source_a source_b tx_backward

{-# ANN topEntity
  (Synthesize
    { t_name = "PacketRoundRobin"
    , t_inputs = [PortName "clk", PortName "rst", PortProduct "source_a" [PortName "payload", PortName "valid", PortName "last"], PortProduct "source_b" [PortName "payload", PortName "valid", PortName "last"], PortName "tx_ready"]
    , t_output = PortProduct "" [PortName "source_a_ready", PortName "source_b_ready", PortProduct "tx" [PortName "payload", PortName "valid", PortName "last"]]
    }) #-}

...
ZLang `0.1.0a4` is an experimental alpha release.  The compiler deliberately
...
- Direct SystemVerilog is the stable supported secondary backend for the
...
- Clash 1.11 is the primary/reference backend and is an optional external tool.
...
- Direct-SV and Clash may support different bounded intersections.  Unsupported
...
- Bounded model checking is reported as `bounded_pass`, never as an unbounded
  proof.
- M35 safety, M36 semantic-reference equivalence, M38 cross-backend evidence,
  and M39 formal-aware selection run only when the required observations,
